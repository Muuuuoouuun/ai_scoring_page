...
## 수정 후 최종 재검증 — 2026-09-12T14:59:10.412Z

root가 `sourceAuthority(c,current)`를 plan/dropBatch/empty-digest SQL에 결합하고 오래된 source의 진입을 보류한 이후, **같은 행위 시험 15개가 모두 통과**했다. 별도 소스 지문 안정성 1개도 통과하여 Node 총 16 통과·0 실패, exit 0이다. source/card/queue 보존, daily 슬롯 미소비, 현재 worker의 정상 발송이라는 기존 기대를 유지했다. 이는 이 보고서의 제한된 race 범위이며 사전 26개 전체나 실제 운영/수신 검증을 뜻하지 않는다.

- [수정 후 TAP](/private/tmp/ais-v15-race-run.fixed.tap), [수정 후 구조화 결과](/private/tmp/ais-v15-race-run.fixed.json)
- [수정 전 RED 보고서](/private/tmp/ais-v15-race-run.red.md), [RED TAP](/private/tmp/ais-v15-race-run.red.tap), [원래 시험](/private/tmp/ais-v15-race-run.red.test.mjs)

세 RED의 재검증 결과: (1) 구버전 matched는 최신 R2 큐를 queued로 유지하고 요청 0, 최신 worker 요청 1; (2) 구버전 digest도 큐를 유지하고 dispatch 행/슬롯을 생성하지 않으며 최신 worker가 즉시 요청 1; (3) currentNotices 이후 pending SELECT 사이 R2가 커밋한 실제 경합에서도 R2 큐 queued, digest 슬롯 null, 구버전 요청 0이었다. 나머지 R2/R3/R8/R9/R10/P6 시험은 계속 통과했다.

처음 수정 후 실행은 digest 단언이 존재하지 않는 dispatch 행의 필드를 읽어 TypeError가 났다. 구버전이 입구에서 보류되어 행 자체를 만들지 않는 것은 ‘슬롯 미소비’를 충족한다. 따라서 **단언 한 줄만 `staleState?.next_digest_not_before ?? null`로 바꿨으며**, 기대한 null/최신 큐 유지/최신 worker sent 1은 그대로다. [그 중간 출력](/private/tmp/ais-v15-race-run.fixed-initial.tap)도 보존했다. 나머지 시험 범위와 assertion은 수정하지 않았다.

최종 실행 중 8개 기록 소스 지문은 동일했다. 수정 후 `email-delivery.ts` SHA256은 `eb4010f2d3f7f3f5fabcbecf80ee24efd3ab7e4b17005830209bca2eacde0abd`, `notice-sources.ts`는 `6b39a91474d28268b0872ecdd93c0994251906e38aedc7e81e0dad03d5128b55`이다. billing 전역 head 분리 변경은 이번 이메일 fixture에 개인 billing 계약이 없으므로 그 별도 개인정보 경계를 검증했다고 확대하지 않는다.
