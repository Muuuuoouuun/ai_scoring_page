...
const p='/private/tmp/ais-v11-relevance-final-regression-2.json';
...
j.gate624Opinion={criterion:'A varied set of realistic purpose/condition queries has evaluated relevance.',assessment:'Evidence supports completion of this evaluation-performed criterion. No gate file or score changed. This is not a statement that all relevance/exclusion behavior passes.',basis:'36 fixed cases span six work domains, explicit negation and product exclusions, positive image boundary, plan/language/owned constraints, unsupported requirements and unregistered purpose; expectations precede outputs; failures and limitations preserved.',remainingFailures:j.observations.filter(o=>o.verdict==='FAIL').map(o=>({id:o.id,checks:o.checks.filter(c=>!c.passed)}))};
